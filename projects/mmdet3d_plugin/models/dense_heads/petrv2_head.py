...
def tensor2img(tensor, path='/data1/nuScenes/visualization/'):
    n = tensor.shape[0]
    channel =256
    channel_num = random_num(25,channel)
    x_ = tensor.reshape(256,20,50,)
    x = x_.cpu()

    plt.figure(figsize=(20, 50))
    for index, channel in enumerate(channel_num):
        ax = plt.subplot(5, 5, index+1,)
        plt.imshow(x[channel, :, :])
    plt.savefig(path+'feature.jpg', dpi=300)
...
def calculate_sim(x1, x2, shift=50):
    C, H, W = x1.shape[-3:]
    w1 = W -shift
    x1 = x1.squeeze().permute(1,2,0)
    x2 = x2.squeeze().permute(1,2,0)
...
class PETRv2Head(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.positional_encoding = build_positional_encoding(
                positional_encoding)
        self.transformer = build_transformer(transformer)   ##### framework
        if self.mvr:
    ...
    def _init_layers(self):
        ...
        #self.tem_embedding = nn.Parameter(torch.randn(2,256))  
        if self.with_fpe:
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        x_new = torch.empty_like(x_ori, device=x_ori.device)
####################### RVM module ########################################
        masked_num  = self.mask_num
        #masked_num = torch.randint(1,6,(1,)).item()  #choose random masked num
        shuffle_indice = torch.rand(6, device=x.device).argsort()
        #shuffle_indice = torch.tensor([3,0,1,5,2,4])  #choose a fixed make method
        mask_ind, unmask_ind = shuffle_indice[:masked_num], shuffle_indice[masked_num:]

        mask_inds = torch.cat((mask_ind, (mask_ind+6)),dim=0)
        unmask_inds = torch.cat((unmask_ind, (unmask_ind+6)),dim=0)
        shuffle_indices = torch.cat((mask_inds,unmask_inds),dim=0)
        x[:,mask_inds] = 0
        ...
        masks = x.new_ones(
            (batch_size, num_cams, input_img_h, input_img_w))   ### new_ones 
...
################################## Local MVR ##########################################################
        if self.mvr:
            ...
            x_m = None
            refine_mask_inds = []
            for i in range(masked_num):
                mask_idx = mask_ind[i].item()
                refine_mask_inds.append(mask_idx)
                refine_mask_inds.append(mask_idx+6)
                if relation_dict[str(mask_idx)][0] in unmask_ind:
                    x_ref_token[0,left_index] = (patches[relation_dict[str(mask_idx)][0]][right_index] + a * patches[relation_dict[str(mask_idx)][0]+6][right_index]) / (1+a)
                    x_ref_token[1,left_index] = (a * patches[relation_dict[str(mask_idx)][0]][right_index] + patches[relation_dict[str(mask_idx)][0]+6][right_index]) / (1+a)
                else :
                    x_ref_token[:,left_index] = x_ref_token[:,left_index] + self.mask_token

                if relation_dict[str(mask_idx)][1] in unmask_ind:
                    x_ref_token[0,right_index] = (patches[relation_dict[str(mask_idx)][1]][left_index] + a * patches[relation_dict[str(mask_idx)][0]+6][left_index]) /(1+a)
                    x_ref_token[1,right_index] = (a * patches[relation_dict[str(mask_idx)][1]][left_index] + patches[relation_dict[str(mask_idx)][0]+6][left_index]) /(1+a)
                else :
                    x_ref_token[:,right_index] = x_ref_token[:,right_index] + self.mask_token
                x_ref_token[:,mid_index] = x_ref_token[:,mid_index] + self.mask_token
                ...
                x_r = x_r.permute(0,2,1).reshape(2,c,h,w)
                if x_m == None:
                    x_m = x_r
                else:
                    x_m = torch.cat((x_m,x_r),dim=0)
            refine_mask_inds = torch.tensor(refine_mask_inds, device=unmask_inds.device)
            unmask_views = x[:,unmask_inds]
            shuffle_indices = torch.cat((refine_mask_inds,unmask_inds),dim=0)

            concat_tokens = torch.cat((x_m, unmask_views.squeeze(0)),dim=0)
            x_new = torch.empty_like(concat_tokens, device=concat_tokens.device)
            x_new[shuffle_indices] = concat_tokens
            x = x_new.unsqueeze(0)
...
#             if rand == 1:
#                 unmask_ind_1 = [0,]
#                 unmask_ind_2 = [2,3,4,5,6,]
#                 unmask_ind_3 = [8,9,10,11]
#                 x = torch.cat((x[:,unmask_ind_1,...].squeeze(0),x_r[0].unsqueeze(0), x[:,unmask_ind_2,...].squeeze(), x_r[1].unsqueeze(0), 
#                 x[:,unmask_ind_3,...].squeeze()), dim=0).unsqueeze(0)

# ######################### front left ##################################################
#             if rand == 2:
#                 unmask_ind_1 = [0,1,]
#                 unmask_ind_2 = [3,4,5,6,7,]
#                 unmask_ind_3 = [9,10,11]
#                 x = torch.cat((x[:,unmask_ind_1,...].squeeze(),x_r[0].unsqueeze(0), x[:,unmask_ind_2,...].squeeze(), x_r[1].unsqueeze(0), 
#                 x[:,unmask_ind_3,...].squeeze()), dim=0).unsqueeze(0)
# ######################### back left #################################################
#             if rand == 4:
#                 unmask_ind_1 = [0,1,2,3]
#                 unmask_ind_2 = [5,6,7,8,9,]
#                 unmask_ind_3 = [11]
#                 x = torch.cat((x[:,unmask_ind_1,...].squeeze(0),x_r[0].unsqueeze(0), x[:,unmask_ind_2,...].squeeze(0), x_r[1].unsqueeze(0), 
#                 x[:,unmask_ind_3,...].squeeze(0)), dim=0).unsqueeze(0)
# ########################  back right ################################################
#             if rand == 5:
#                 unmask_ind_1 = [0,1,2,3,4,]
#                 unmask_ind_2 = [6,7,8,9,10]
...
###################################################################################################
        ...
        return outs, (x_new.unsqueeze(0), x_ori)
    ...
    def mvrloss(self, x, x_ori):
        b,n,c,h,w = x.shape
        loss = (x - x_ori) ** 2
        loss = 0.05 * loss.sum()/(c*h*w)
        loss_dict = {'loss_mvr':loss
            }
        return loss_dict
