...
#from .MAE_ import DETRTransformer
...
class PETRTransformer(BaseModule):
    ...
    def __init__(self, encoder=None, decoder=None, init_cfg=None, cross=False):
        ...
        self.cross = cross
        #self.attention = nn.MultiHeadAttention(d_model=256, n_head=8)  ##### 
...
#########################################################################################################
    ...
    def forward(self, x, mask, query_embed, pos_embed ,reg_branch=None):
        ...
        # value_tem = value_tem.permute(1, 3, 4, 0, 2).reshape(-1, bs, c)
        # ################## #####################
        # # num_fea = memory.shape[0]  #### 12000
        # # a =0.8
        # # #print(memory[0: num_fea//2].shape)
        # # memory[0:6000,...] = a * memory[0:6000,...]
        # #################################################
...
class PETRMultiheadAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = nn.MultiheadAttention(embed_dims, num_heads, attn_drop,
                                          **kwargs)
        #self.selfattn = nn.MultiheadAttention(embed_dims, num_heads, attn_drop)
        #self.DETRTransformer = DETRTransformer(256,2048) 
...
##################################################################
            # key_before = key_before.transpose(0,1)
            # key_now = key_now.transpose(0,1)
            # value_before = value_before.transpose(0,1)
            # value_now = value_now.transpose(0,1)


        # out_before = self.attn(
        #     query=query,
        #     key=key,
        #     value=value,
        #     attn_mask=attn_mask,
        ...
        #     value=value_now,
        #     attn_mask=attn_mask,
        #     key_padding_mask=key_padding_mask[:,6000:])[0]
...
######################################################################
        ...
        #out = self.DETRTransformer(out)

        if self.batch_first:
            out = out.transpose(0, 1)
        # print(out.shape)  (900,1,256)
